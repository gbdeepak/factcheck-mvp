...
export default function DocxViewerPage() {
  ...
  // Component mount effect and URL parameter extraction - exactly like the working version
  useEffect(() => {
    setIsMounted(true);
    console.log('[DOCXViewer] Component mounted');
    
    // Extract URL parameters immediately - exactly like the working version
  ...
  // Load DOCX when component is ready and highlightParams are set - exactly like the working version
  useEffect(() => {
  ...
  const addHighlight = () => {
    ...
    console.log('[DOCXViewer] Container has content:', containerRef.current?.innerHTML.length || 0, 'characters');
  ...
  const getDocumentName = () => {
    if (!highlightParams?.docxUrl) return 'Unknown Document';
    const urlParts = highlightParams.docxUrl.split('/');
